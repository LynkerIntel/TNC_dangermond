...
def time_format(dt_time=dt.datetime.now(), time_type="local"):
    if time_type == "utc":
        str_time = dt.datetime.strftime(
            dt_time, "%Y-%m-%dT%H:%M:%SZ"
        )  # "%Y-%m-%dT%H:%M:%S.%f"
    else:
        str_time = dt.datetime.strftime(
            dt_time, "%Y-%m-%dT%H:%M:%S"
        )  # "%Y-%m-%dT%H:%M:%S.%f"
    return str_time
...
def authenticate(email):
    data = {"email": email, "strategy": "local", "password": getpass()}
...
def list_stations(orgslug="all", query_add="none"):
    ...
    query = {"$sort[name]": 1, "$select[name]": 1, "$select[slug]": 1, "$limit": 2016}
...
def list_datastreams_by_medium_variable(
    medium="", variable="", aggregate="", station_id="", orgslug="", query_add=""
):
    # parameters:
...
def list_datastreams_by_measurement(
    measurement="", aggregate="", station_id=[], orgslug="", query_add=""
):
...
def get_datapoints(
    ...
    # Rename columns
    df.rename(
        columns={"lt": "timestamp_local", "t": "timestamp_utc", "v": datastream_name},
        inplace=True,
    )
...
def get_datapoints_from_id_list(
    datastream_id_list, begins_at, ends_before=time_format(), time_type="local"
):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for dsid in datastream_id_list:
            ...
            future = executor.submit(
                get_datapoints, dsid, begins_at, ends_before, time_type, "default"
            )
        ...
        for future in concurrent.futures.as_completed(dftemp_list):
            ...
            if type(dftemp) is int:
                print(
                    j,
                    "ERROR: datastream failed to retrieve. check authentication or ID("
                    + datastream_id_list[j]
                    + ")",
                )
                continue
            elif dftemp.empty:
                print(
                    "datastream ID("
                    + datastream_id_list[j]
                    + ")  has no data for this time period. Skipping."
                )
                continue
...
def get_datapoints_from_station_id(
    station_id, begins_at, ends_before=time_format(), time_type="local"
):
...
def lookup_datapoints(query, endpoint="datapoints/lookup", interval=5):
    ...
    max_records = pd.date_range(
        start=begins_at_original,
        end=ends_before_original,
        tz="UTC",
...
def __main():
    ...
    if bdatapoints == True:
        ...
        print(
            "get_datapoints end date set to now, count:",
            len(dd),
            "min date:",
            dd.index.min(),
            "max date:",
    ...
    if bdatapoints_lookup == True:
        ...
        for doc in bigjson:
            print(
                doc["name"],
                len(doc["datapoints"]["data"]),
                doc["datapoints"]["limit"],
                doc["_id"],
            )
